...
std::vector<Complex> lpmCF::logCF(Complex &u, std::vector<double > &p, std::vector<std::unordered_map<std::string, double> > &l, std::vector<std::vector<double> > &w, double lambda, double q){
	int n=p.size();
	int m=w[0].size();
	...
	v[0]=Complex(0, 0);
	//lgdCF phiL(l[0]["exposure"], l[0]["alpha"], l[0]["beta"], l[0]["sigma"], l[0]["tau"],l[0]["x0"]);
	lgdCF phiL;
	for(int i=0; i<n;i++){

		//lgdCF phiL(l[i]["exposure"], l[i]["alpha"], l[i]["beta"], l[i]["sigma"], l[i]["tau"],l[i]["x0"]);

		helperPhi[i]=phiL.execute(upperU, l[i]["exposure"], l[i]["alpha"], l[i]["beta"], l[i]["sigma"], l[i]["tau"],l[i]["x0"]).subtract(1.0).multiply(p[i]);//e^{i*u*l_j}-1
